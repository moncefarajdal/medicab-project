...
export class ConsultationComponent implements OnInit {
    ...
    typePaiements: Array<TypePaiementVo>
    patients: Array<PatientVo>;
    mutuelles: Array<MutuelleVo>;

    selectedConsultations: ConsultationVo[];
    deleteConsultationsDialog: boolean = false;
    ...
    ngOnInit(): void {
        ...
        this.breadcrumbItems.push({ label: 'Dashboard', routerLink:'/assistant/dashboard' });
        this.breadcrumbItems.push({ label: 'Consultations', routerLink:'/assistant/consultation' });
    }
    ...
    public async deleteSelectedConsultations() {
        ...
        if (isPermistted) {
            this.confirmationService.confirm({
                ...
                accept: () => {
                    this.selectedConsultations.forEach( (e) => {
                            this.consultationService.delete(e).subscribe(status => {
                                if (status > 0) {
                                    const position = this.consultations.indexOf(e);
                ...
                accept: () => {
                    this.consultationService.delete(consultation).subscribe(status => {
                        if (status > 0) {
                            ...
                            position > -1 ? this.consultations.splice(position, 1) : false;
                            this.messageService.add({severity: 'success', summary: 'Successful', detail: 'Consultation supprimée', life: 3000});
                        }
                    }, error => console.log(error));
                }
            });
