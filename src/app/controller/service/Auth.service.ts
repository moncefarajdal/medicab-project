...
export class AuthService {
    ...
    // private _authenticated = <boolean>JSON.parse(localStorage.getItem('autenticated'));
    ...
    // public loginAssistant(username: string, password: string) {
    //     this.http.post<any>(this.API + 'login', {username, password}, {observe: 'response'}).subscribe(
    //         resp => {
    //             this.error = null;
    //             const jwt = resp.headers.get('Authorization');
    ...
    //             console.log('you are logged in successfully');
    //             console.log(resp);
    //             this.router.navigate(['/' + environment.rootAppUrl + '/assistant/dashboard']);
    //         }, (error: HttpErrorResponse) => {
    //             this.error = error.error;
    //             console.log(error);
    //         }
    ...
    public loginAssistant(username: string, password: string) {
        this.http.post<any>(this.API + 'login', {username, password}, {observe: 'response'}).subscribe(
            resp => {
                ...
                console.log('you are logged in successfully');
                // this.router.navigate(['/' + environment.rootAppUrl + '/assistant/dashboard']);
                this.router.navigate(['/' + environment.rootAppUrl + '/' +
                localStorage.getItem('role').substr(5, 20).toLocaleLowerCase() +'/dashboard']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
    ...
    public loginMedecin(username: string, password: string) {
        this.http.post<any>(this.API + 'login', {username, password}, {observe: 'response'}).subscribe(
            resp => {
                ...
                this.loadInfos();
                console.log('you are logged in successfully as medecin');
                this.router.navigate(['/' + environment.rootAppUrl + '/medecin/consultation']);
            }, (error: HttpErrorResponse) => {
                this.error = error.error;
                console.log(error);
            }
    ...
    public loadInfos() {
        ...
        console.log(this._authenticatedUser.roles);
    ...
    public authenticatedUserByAssistant(): ChercheurVo {
        if (localStorage.getItem(this.connectedChercheur) !== null) {
            return JSON.parse(localStorage.getItem(this.connectedChercheur));
        }
        return null;
